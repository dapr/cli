...
func metakeys(ctx context.Context, opts DBOptions) ([]string, error) {
	if !dclient.IsSQLDriver(opts.Driver) {
		// Handle redis & mongodbo
		return nil, fmt.Errorf("unsupported state store driver %s", opts.Driver)
	}

	if opts.ConnectionString == nil {
		return nil, fmt.Errorf("connection string is required for SQL drivers")
	}
