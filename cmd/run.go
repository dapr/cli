...
func init() {
	...
	RunCmd.Flags().IntVarP(&port, "dapr-http-port", "", -1, "the HTTP port for Dapr to listen on")
	RunCmd.Flags().IntVarP(&grpcPort, "dapr-grpc-port", "", -1, "the gRPC port for Dapr to listen on")
